...
class PackageService {
  ...
  public createPackageEntityWithoutKeys = async (packageCreateData: { name: string, activities: string[], description: string, price: number, startDate: Date, endDate: Date, count: number, maxPersons: number, locationId: number, adminId: number  }) => {
    const repository = this.getRepository();
    return repository.create({
      name: packageCreateData.name,
      activities: packageCreateData.activities,
  ...
  public updatePackage = async (packageId: number, packageEntity: PackageEntity) => {
    const repository = this.getRepository();
    return repository
